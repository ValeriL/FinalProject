...
1. Длительное время предобработки тренировочного датасета, а впоследствии и собранных с сайта отзывов для анализа.
    * была убрана проверка синтаксиса (на точности предсказания не отразилось).
    * заменены функции с использованием регулярных выражений для замены окончаний и т.п. на более быстрый лемматайзер, выполняющий те же функции.
    * сохранение "очищенного" датасета для экспериментов с разными алгоритмами.
2. Отсутствие открытого API у англоязычных сервисов по просмотру фильмов.
    * использовалось стороннее API (см.п. Приложение-Технологии)
    * но есть минус, который не удалось решить - ограничение по запросам в день (в бесплатной версии API).
3. Страницы с отзывами, переход на которые осуществляется с помощью ключей пагинации.
    * можно переходить на определенную страницу, по указанному ключу с помощью того же API, но это уже новый запрос. А на их количество стоит ограничение, поэтому все отзывы скачать получится не у всех фильмов.
    * в данном проекте поставлено ограничение на максимальное число скачиваемых отзывов.
4. И по мелочи, к примеру,
    * одному фильму на сайте соответствовало несколько id (проверка на повторяющиеся названия).
